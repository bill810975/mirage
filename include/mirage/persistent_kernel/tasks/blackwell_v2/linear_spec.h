...
//   [+12 ..+17]  mma_mbar        (count=1,            launcher→loader, shared
//   W/A empty)
...
struct ChannelSpec {
  ...
  Role consumer;
  Arrival consumer_kind;   // frees empty[] = the release edge
...
inline constexpr ChannelSpec CHANNELS[] = {
    ...
    {"Acc",
     ...
     /*shares_empty_with=*/-1,
     /*tmem=*/true,
     /*reinit_full_by=*/Role::Launcher,
     /*reinit_empty_by=*/Role::Launcher},
};
...
inline constexpr OneShotSem ONESHOT_SEMS[] = {
    ...
    {"consumer_done",
     SEM_CONSUMER_DONE,
     4 * WARP_SIZE,
     Role::Launcher}, // consumer→launcher
};
