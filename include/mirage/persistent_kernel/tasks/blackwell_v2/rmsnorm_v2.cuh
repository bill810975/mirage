...
//   0 = TMA load (input/weight), 1 = compute (read input/weight, write
//   output/reduce), 2 = store (read output)
...
struct RmsNormBuffers {
  ...
  __device__ explicit RmsNormBuffers(char *base,
...
__device__ __noinline__ void
    rms_norm_task(mirage::runtime::TaskDesc const *task_desc,
  ...
  for (int for_idx = 0; for_idx < NUM_TILES; for_idx++) {
    if (for_idx + 1 < NUM_TILES) {
      load_smem<T, BYTES_PER_CP>(
          shared_input_buffer + threadIdx.x * CHUNK_SIZE +
              (for_idx + 1) * TILE_SIZE,
...
struct RmsNormConsumer {
  ...
  __device__ __forceinline__ static void
      run(mirage::runtime::TaskDesc const *task_desc,
          ...
          void *runtime_smem,
          int) {
    rms_norm_task<T, BATCH_SIZE, HIDDEN_DIM>(task_desc,
