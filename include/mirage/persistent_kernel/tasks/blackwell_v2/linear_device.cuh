...
__device__ __forceinline__ void linear_init(int dyn_sem_base) {
  LIN_INIT_FULL(dyn_sem_base, CH_W);
  LIN_INIT_EMPTY(dyn_sem_base, CH_W);
  LIN_INIT_FULL(dyn_sem_base, CH_A);
  LIN_INIT_EMPTY(dyn_sem_base, CH_A);
  LIN_INIT_FULL(dyn_sem_base, CH_ACC);
  LIN_INIT_EMPTY(dyn_sem_base, CH_ACC);
  mbarrier_init(dyn_sem_base + ONESHOT_SEMS[0].sem * 8,
                ONESHOT_SEMS[0].arrivals);
  mbarrier_init(dyn_sem_base + ONESHOT_SEMS[1].sem * 8,
                ONESHOT_SEMS[1].arrivals);
...
// fence relative to the surrounding TMA/MMA issue, re-exposing the
// stale-arrival race. Inlining keeps the fence ordered as written.
#define LIN_REINIT_FULL(dyn, K, r)                                             \
  do {                                                                         \
    if (CHANNELS[K].reinit_full_by == (r))                                     \
      LIN_INIT_FULL(dyn, K);                                                   \
  } while (0)
#define LIN_REINIT_EMPTY(dyn, K, r)                                            \
  do {                                                                         \
    if (CHANNELS[K].reinit_empty_by == (r))                                    \
      LIN_INIT_EMPTY(dyn, K);                                                  \
  } while (0)
__device__ __forceinline__ void reinit_for_role(Role r, int dyn_sem_base) {
  LIN_REINIT_FULL(dyn_sem_base, CH_W, r);
  LIN_REINIT_EMPTY(dyn_sem_base, CH_W, r);
  LIN_REINIT_FULL(dyn_sem_base, CH_A, r);
  LIN_REINIT_EMPTY(dyn_sem_base, CH_A, r);
  LIN_REINIT_FULL(dyn_sem_base, CH_ACC, r);
  LIN_REINIT_EMPTY(dyn_sem_base, CH_ACC, r);
