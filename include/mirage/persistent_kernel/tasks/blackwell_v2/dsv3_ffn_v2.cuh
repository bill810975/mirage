...
// Multi-role epilogue/prologue helpers. op_sem_addr < 0 => stage-1
// (consumer-only variant): all handshakes compile to the plain consumer
// barrier.
...
// ============================================================================
// T2 — topk_sigmoid. v1 Phase C ported to the 4 consumer warps (128 threads):
// warp w handles groups {w, w+4} where v1 used one warp per group — the
// per-group shfl trees are warp-local, so per-group values are bit-exact.
//   inputs : [0] inter f32[ROUTER_N*RKSPLIT]
//            [1] bias f32[ROUTER_N] (e_score_correction_bias)
//            [2] logits bf16[ROUTER_N] (hidden write — compare artifact,
//                exactly v1's sc.logits)
//   outputs: [0] meta i32[META_INTS]
//   params baked at registration: local_expert_start, num_local_experts,
...
#pragma unroll
  ...
  // EP-local filter (verbatim) + publish meta.
  if (threadIdx.x == 0) {
...
#pragma unroll
    ...
    meta[META_OFF_COUNT] = active_count;
    meta[META_OFF_MAGIC] = META_MAGIC;
#pragma unroll
    for (int s = 0; s < MAX_ACTIVE; s++) {
      meta[META_OFF_EXPERTS + s] = me[s];
      meta[META_OFF_WEIGHTS + s] = __float_as_int(mw[s]);
    }
#pragma unroll
