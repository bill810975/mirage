...
__device__ inline void tma_3d_load_l2(int dst,
                                      void const *tmap_ptr,
                                      int x,
                                      int y,
                                      int z,
  ...
  asm volatile("cp.async.bulk.tensor.3d.shared::cluster.global.mbarrier::"
               "complete_tx::bytes.cta_group::1.L2::cache_hint "
               "[%0], [%1, {%2, %3, %4}], [%5], %6;" ::"r"(dst),
               "l"(tmap_ptr),
               "r"(x),
               "r"(y),
...
__device__ inline void tcgen05_mma(
    int taddr, uint64_t a_desc, uint64_t b_desc, uint32_t idesc, int enable_d) {
  asm volatile(
      "{\n\t.reg .pred p;\n\t"
      "setp.ne.b32 p, %4, 0;\n\t"
...
//
// One TaskDesc = one output tile. tile_idx encodes both spatial_idx and
// k_slice:
...
__device__ __noinline__ void
  ...
  int const W_tma_mbar_base = dyn_sem_base + SEM_W_TMA_BASE * 8;
  int const A_tma_mbar_base = dyn_sem_base + SEM_A_TMA_BASE * 8;
  int const mma_mbar_addr = dyn_sem_base + SEM_MMA_BASE * 8;

  int const M_mma = N_real;
  int const grid_m = M_mma / BLOCK_M;
  int const num_spatial_tiles = grid_m;
  int const total_k_iters = K / BLOCK_K;
  int const iters_per_slice = total_k_iters / SPLIT_K;
  int const num_tiles = num_spatial_tiles * SPLIT_K;

  if (tile_idx >= num_tiles) {
    return;
  }

  int const tiles_left = num_tiles - tile_idx;
  int const tiles_to_process =
      (TILES_PER_TASK < tiles_left) ? TILES_PER_TASK : tiles_left;
  ...
  for (int s = 0; s < NUM_STAGES; s++) {
    asm volatile(
        "mbarrier.init.shared::cta.b64 [%0], 1;" ::"r"(mma_mbar_addr + s * 8));
  ...
  for (int t = 0; t < tiles_to_process; t++) {
    ...
    int const cur_spatial_idx = cur_tile_idx % num_spatial_tiles;
    int const cur_k_slice = cur_tile_idx / num_spatial_tiles;
    int const cur_k_start = cur_k_slice * iters_per_slice;
    ...
    for (int i = 0; i < iters_per_slice; i++) {
      ...
      int const W_smem =
          smem + task_desc->smem_region_offset(
...
__device__ __noinline__ void
  ...
  int const A_tma_mbar_base = dyn_sem_base + SEM_A_TMA_BASE * 8;
  int const mma_mbar_addr = dyn_sem_base + SEM_MMA_BASE * 8;
  int const mainloop_mbar_addr = dyn_sem_base + SEM_MAINLOOP_BASE * 8;
  int const epilogue_mbar_addr = dyn_sem_base + SEM_EPILOGUE_BASE * 8;
  int const tmem_ready_mbar_addr = dyn_sem_base + SEM_TMEM_READY * 8;
  int const consumer_done_mbar_addr = dyn_sem_base + SEM_CONSUMER_DONE * 8;
  ...
  int const scratch_smem_addr =
      smem +
      task_desc->smem_region_offset(::kernel::linear_sm100_v2::REGION_SCRATCH);
  asm volatile(
    ...
    for (int t = 0; t < tiles_to_process; t++) {
      ...
      for (int i = 0; i < iters_per_slice; i++) {
        ...
        int const A_smem =
            smem + task_desc->smem_region_offset(
                       ::kernel::linear_sm100_v2::REGION_A_0 + tma_stage);
        int const tmem = taddr + mainloop_stage * BLOCK_N;
...
__device__ __noinline__ void
  ...
  for (int t = 0; t < tiles_to_process; t++) {
    ...
    if (n_real < N_real) {
      int const t_col = taddr + mainloop_stage * BLOCK_N;
...
#pragma unroll
        for (int m = 0; m < M_REAL; m++) {
          ...
          asm volatile("st.global.L1::no_allocate.b16 [%0], %1;" ::"l"(
                           C_ptr + m * N_real + n_real),
                       "h"(*(uint16_t *)&val)
                       : "memory");
        }
      } else {
...
#pragma unroll
        for (int m = 0; m < M_REAL; m++) {
          asm volatile("st.global.L1::no_allocate.b32 [%0], %1;" ::"l"(
                           ws_base + m * N_real + n_real),
                       "f"(f[m])
                       : "memory");
        }
      }
...
template <int SPLIT_K = 1, int TILES_PER_TASK = 1>
__device__ __noinline__ void
    linear_storer_task(mirage::runtime::TaskDesc const *task_desc,
                       mirage::runtime_v2::RuntimeSMEM *runtime_smem,
                       int N_real,
                       int K_param,
                       int tile_idx,
  ...
  int const grid_m = M_mma / BLOCK_M;
  int const num_spatial_tiles = grid_m;
  int const total_k_iters = K_param / BLOCK_K;
  int const iters_per_slice = total_k_iters / SPLIT_K;
  int const num_tiles = num_spatial_tiles * SPLIT_K;
  ...
  int const tiles_to_process =
      (TILES_PER_TASK < tiles_left) ? TILES_PER_TASK : tiles_left;
  int const total_iters = tiles_to_process * iters_per_slice;
...
#pragma unroll
  for (int s = 0; s < NUM_STAGES; s++) {
    auto const &W =
        task_desc->smem_regions[::kernel::linear_sm100_v2::REGION_W_0 + s];
    for (int p = 0; p < W.page_count; p++) {
      page_remaining[W.physical_page_start + p]++;
    }
    auto const &A =
        task_desc->smem_regions[::kernel::linear_sm100_v2::REGION_A_0 + s];
  ...
  {
    auto const &SC =
        task_desc->smem_regions[::kernel::linear_sm100_v2::REGION_SCRATCH];
  ...
  for (int it = 0; it < total_iters; it++) {
    ...
    if (fires_per_stage[stage] == last_use[stage]) {
      ...
      auto const &W =
          task_desc
              ->smem_regions[::kernel::linear_sm100_v2::REGION_W_0 + stage];
      for (int p = 0; p < W.page_count; p++) {
        int phys = W.physical_page_start + p;
        if (--page_remaining[phys] == 0) {
          mirage::runtime_v2::runtime_finish_page(runtime_smem, phys, 1);
        }
      }
      auto const &A =
          task_desc
              ->smem_regions[::kernel::linear_sm100_v2::REGION_A_0 + stage];
      for (int p = 0; p < A.page_count; p++) {
...
__device__ __forceinline__ void splitk_reduce_task(float *workspace,
                                                   nv_bfloat16 *C_ptr,
                                                   nv_bfloat16 const *res_ptr,
                                                   int N_real,
                                                   int bid,
  ...
  if (threadIdx.x >= EFF_THREADS) {
    return;
  }
  int const total = M_REAL * N_real;
  for (int idx = bid * EFF_THREADS + threadIdx.x; idx < total;
