...
inline constexpr int
    total_smem_bytes(int t_size_bytes, int hidden_dim, int num_threads) {
...
inline ::mirage::runtime::TaskSmemInfo
  ...
  info.regions.push_back({"input",
                          ...
                          /*release_step=*/2,
                          /*contiguous=*/true});
  info.regions.push_back({"weight", buf_bytes, 1024, -1, true, 2, true});
  info.regions.push_back({"output", buf_bytes, 1024, -1, true, 2, true});
  info.regions.push_back({"reduce", reduce_bytes, 1024, -1, true, 2, true});
  return info;
}
