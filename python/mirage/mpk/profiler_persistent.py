...
event_name_list = {
    ...
    # NOTE: upstream also had 204: "V2_PREPARE_BATCH", dropped here because
    # 204 is TASK_SCHD_PREPARE_BATCH on this branch (v1 labeling wins).
    205: "V2_ITER_SYNC",
    206: "V2_GO_WAIT",
    207: "V2_DEP_WAIT",
    208: "V2_PAGE_WAIT",
    ...
    # v2/v3 task types (renumbered in the v2 merge: upstream 246/247/281..286
    # collide with pre-existing ids on this branch; see runtime_header.h).
    244: "TASK_LINEAR_SM100_V3",
    245: "TASK_LINEAR_WITH_RESIDUAL_SM100_V3",
    242: "TASK_LINEAR_SM100_V2",
    243: "TASK_LINEAR_WITH_RESIDUAL_SM100_V2",
    326: "TASK_RMS_NORM_HOPPER_V2",
    327: "TASK_SILU_MUL_V2",
    328: "TASK_EMBEDDING_V2",
    329: "TASK_ATTN_SM100_V2",
    330: "TASK_ARGMAX_PARTIAL_SM100_V2",
    331: "TASK_ARGMAX_REDUCE_SM100_V2",
}
...
def export_to_perfetto_trace(
    ...
    for block_idx, group_idx, event_idx, event_no, event_type, timestamp in events:
        if num_groups >= 5 and event_idx not in event_name_list:
            # v2: junk/reserved entries (e.g. the dep-spin accumulators at the
            # buffer tail) — skip rather than garbage the trace. v1 keeps the
            # legacy UNKNOWN_<id> labeling so nothing is silently dropped.
            continue
