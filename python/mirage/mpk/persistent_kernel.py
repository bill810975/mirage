...
class PersistentKernel:
    ...
    def moe_topk_sigmoid_routing_layer(
        ...
        bias: DTensor,          # [num_experts] e_score_correction_bias
        output: tuple,          # (topk_weights, routing_indices, masks)
        grid_dim: tuple,
        block_dim: tuple,
    ):
        """TopK sigmoid routing for DeepSeek V3 MoE.

        Uses sigmoid scoring (not softmax) with correction bias for expert
        selection, then renormalizes the top-k weights.
        """
        assert input.num_dims == 2   # (batch_size, num_experts)
        ...
        self.kn_graph.register_task(tb_graph, "moe_topk_sigmoid_sm100")
